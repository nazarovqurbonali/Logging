...
public class CategoryService : ICategoryService
{
    ...
    public async Task<Response<List<GetCategoryDto>>> GetCategoriesAsync()
    {
        try
        {
            _logger.LogDebug("Starting method GetCategoriesAsync in time: {DateTimeNow}", DateTime.Now.Millisecond);
            var categories = await _context.Categories.Select(x => new GetCategoryDto()
            {
                Name = x.Name,
                Id = x.Id,
            }).ToListAsync();
            _logger.LogDebug("Finished method GetCategoriesAsync in time: {DateTimeNow}", DateTime.Now.Millisecond);
